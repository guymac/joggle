...
/**
 * Keeps track of player state. Intended to be used through action or mouse
 * event handlers.
 */
public class JogglePlayer extends JoggleGame
{
    protected boolean[][] state;
    protected StringBuffer word;
    protected Vector <int[]> selected;
    ...
    public boolean select(int row, int col)
    {
        ...
        {
            if (debug)
                System.out.println(word.toString() + " has been terminated.");
            /*
             * tile already selected, so this is a word-terminating action
             * 
             * if the word is long enough and it's in the list add the point value to the
             * total and score
             */
            if (word.length() > 2)
            {
        ...
        {
            /*
             * tile has not been selected if it's selectable, select it else they're
             * starting over so reset
             */

            if (selected.size() > 0)
            {
                if (debug)
                    System.out.println("Continuing word " + word.toString());
                // (n+1)th selection
                ...
                if (debug)
                    System.out.println("dx = " + dx2 + ", dy = " + dy2 + ".");
            ...
            if (debug)
                System.out.println("Word is now " + word.toString());
        }
