...
public class JoggleBoard implements Debuggable {
  ...
  /**
   * Roll each die and swap each randomly,
   * preserving each die instance.
   */
  public void shuffle() {
    try {
  ...
  public boolean recurse(boolean[][] state, boolean found, String word, int pos, int row, int col) {
    if(debug) System.out.print(""+pos+". ("+row+","+col+")");
    ...
    if(state[row][col]) {
      if(debug) System.out.println(" has already been selected!");
      return false;
    }
    ...
    if(word.charAt(pos)=='q') { 
      ...
      ++pos;
    } else look=word.substring(pos,1+pos);
    if(debug) System.out.println(" = "+value+" ?= "+look+".");
    if(!(value.equals(look))) return false;
    state[row][col]=true;
    // corner tiles
    if(row==0 && col==0) {
      if(debug) System.out.println("Upper left.");
      ...
      if(debug) System.out.println("Upper right.");
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col);
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col+1);
      if(!found) found=recurse(state, found,  word, 1+pos, row, col+1);
    ...
    // side tiles
      ...
      if(debug) System.out.println("Left column");
      if(!found) found=recurse(state, found,  word, 1+pos, row, col-1);
      if(!found) found=recurse(state, found,  word, 1+pos, row, col+1);
      if(!found) found=recurse(state, found,  word, 1+pos, row+1, col-1);
      if(!found) found=recurse(state, found,  word, 1+pos, row+1, col);
      ...
      if(debug) System.out.println("Right column");
      if(!found) found=recurse(state, found,  word, 1+pos, row, col-1);
      if(!found) found=recurse(state, found,  word, 1+pos, row, col+1);
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col-1);
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col);
      ...
      if(debug) System.out.println("Top row");
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col);
      if(!found) found=recurse(state, found,  word, 1+pos, row+1, col);
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col+1);
      if(!found) found=recurse(state, found,  word, 1+pos, row, col+1);
      ...
      if(debug) System.out.println("Bottom row");
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col);
      if(!found) found=recurse(state, found,  word, 1+pos, row+1, col);
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col-1);
      if(!found) found=recurse(state, found,  word, 1+pos, row, col-1);
    ...
    // center tiles
      ...
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col-1); // 0
      if(!found) found=recurse(state, found,  word, 1+pos, row, col-1); // 1
      if(!found) found=recurse(state, found,  word, 1+pos, row+1, col-1); // 2
      if(!found) found=recurse(state, found,  word, 1+pos, row+1, col);  // 3
      if(!found) found=recurse(state, found,  word, 1+pos, row+1, col+1); // 4
      if(!found) found=recurse(state, found,  word, 1+pos, row, col+1); // 5
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col+1); // 6
      if(!found) found=recurse(state, found,  word, 1+pos, row-1, col); // 7
    }
    state[row][col]=false;
    return found;
  }
  ...
  public boolean has(final String word) {
    if(debug) System.out.println("Checking board for "+word);

    for(int i=0;i<rows;i++)
      for(int j=0;j<cols;j++) 
	if(recurse(new boolean[rows][cols], false, word, 0, i, j)) return true;
    return false;
  }
